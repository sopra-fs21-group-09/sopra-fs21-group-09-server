...
public class TaskService extends AService{
    ...
    public Deadline getDeadline(Long id) {
        if (deadlineRepository.findById(id).isEmpty()) {
            String errorMessage = "deadline was not found";
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, errorMessage);
        }
        return deadlineRepository.findById(id).get();
    }
    ...
    public void updateTask(Long id, Task changesToTask) {
        ...
        var taskToBeUpdated = getTask(id);

        var changesToDeadline = changesToTask.getDeadline();
        if (deadlineRepository.findById(id).isPresent() && changesToDeadline != null) {
    ...
    private void updateDeadline(Long id, Deadline changesToDeadline) {
        ...
        var deadlineToBeUpdated = getDeadline(id);

        changesToDeadline.setTask(getTask(id));
